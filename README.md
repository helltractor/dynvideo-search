...
- 结果列表实时刷新：每行以 B 站视频链接样式渲染为 **[视频标题]** 超链接（悬停可查看 BV 号），点击在新标签页打开视频
- 支持逐条勾选与一键全选（配合筛选可「勾选可见项」），**复制 / 导出仅处理勾选项**，未勾选时作用于全部；勾选状态在列表刷新与筛选切换间保持
...
5. 点击列表中的「[视频标题]」链接即可在新标签页打开对应视频（悬停可查看 BV 号）；条目较多时可用筛选框按 BV 号 / 标题过滤
6. 勾选需要的条目（点击行内任意处或复选框，工具行可一键全选），再点击「复制 BV」/「导出 JSON」即只处理勾选项；未勾选任何条目时处理全部
